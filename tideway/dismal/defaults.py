...
playback_data_filename      = "playback_data.txt"
...
disk_alerts_cmd         = 'df -h | awk \'NR > 1 {OFS=",";print $6,$5}\''
...
health_check_cmd        = 'tw_run $TIDEWAY/python/misc/health_check'
...
playback_data_cmd       = 'find /usr/tideway/var/pool -name .no-expiry | wc -l'
reasoning_cmd           = 'tw_reasoningstatus --waiting-full'
reports_model_cmd       = 'tw_check_reports_model'
resolv_conf_cmd         = 'cat /etc/resolv.conf'
rsyslog_cmd             = 'if command -v systemctl > /dev/null 2>&1; then systemctl is-active rsyslog; else /sbin/service rsyslog status; fi'
...
uname_cmd               = 'uname -a'
vmware_tools_cmd        = 'if command -v systemctl > /dev/null 2>&1; then systemctl is-active vmware-tools; else /sbin/service vmware-tools status; fi'
